...
export const bindEvents = (): (() => void) => {
  ...
  electron.ipcRenderer.on(Events.RUNNER_STATUS_CHANGED, (_, id: number, status: RunnerStatus) => {
    setRunnerStatus(id, status)
  })
  electron.ipcRenderer.on(Events.RUNNER_ACTION, (_, id: number, action: ActionReport) => {
